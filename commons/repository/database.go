...
func MongoDatabase(mongo_uri string, database string) *mongo.Database {
	...
	clientURI := options.Client().ApplyURI(mongo_uri)
	...
	if err != nil {
		logger.Panic("error when trying connect to mongo database -> %v", err)
	}

	// Checking the connection to database
	if errPing := client.Ping(cxtTimeout, nil); errPing != nil {
		logger.Panic("error ping database -> %v", errPing)
	}
